...
<script>
    $(function () {
        //-----------------------------------------------datagrid条件查询数据---------------------------------------
        /**
         * 1.layout方法的使用,expant方法,展开布局
         * 2.jquery表单序列化$(selector).serializeArray()方法,此方法
         *      返回的 JSON 对象是由一个对象数组组成的，其中每个对象包含一个或两个名值对 —— name 参数和 value 参数（如果 value 不为空的话）
         * 3.过滤查询的后台,即传递个后台过滤条件,使用map接收并遍历拼接过滤查询的sql语句
         * 4.根据字段排序,
         *      设置需要排序的自动的sortable属性,
         *      设置后点击该字段的三角标识会异步发送请求,携带sort和order两个参数
         *      sort:代表排序的字段
         *      order:代表排序的方式(如升降序)
         * 5.关于后台获取参数为null时,设为空字符串,sql语句再判断为空字符串不拼接.(其实sql要判断不为null,不为空字符串.提前判了)
         */
        ...
        $("#searchbtn").click(function () {
            $("#mydata").datagrid("load",serializeForm($("#mysearch")));
        })
        $("#clearbtn").click(function () {
            $("#mysearch").form("clear");
            $("#mydata").datagrid("load",{});//load加载,传递参数为空对象,即没有过滤信息
        })

    })
    //js方法：序列化表单
    function serializeForm(form){//传递的是一个jquery对象
        var obj = {};
...
<body>
<div class="easyui-layout" id="lay" style="width: 100%;height:100%;">
    <div region="north" style="height: 100px" title="用户查询" collapsed="true">
        <form id="mysearch" method="post">
            用户名:<input name="username" value=""/>&nbsp;&nbsp;&nbsp;
            开始日期:<input name="startTime" class="easyui-datetimebox" editable="false" style="width: 160px" value=""/>&nbsp;
            结束日期:<input name="endTime" class="easyui-datetimebox" editable="false" style="width: 160px" value=""/>&nbsp;&nbsp;&nbsp;
...
//                    $("#form1").form("clear");//执行form表单对象的clear方法,但是和第一种方法一样,radio中的value也会被清空
                ...
                iconCls: "icon-search",
                handler: function () {
                    $("#lay").layout("expand","north");//使north布局展开
                }
            }],
